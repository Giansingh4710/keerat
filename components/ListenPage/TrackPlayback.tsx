...
import {useModalStore, useStore} from '@/utils/store';
...
export default function TrackPlayback({audioRef}: TrackPlaybackProps): JSX.Element {
  const {nextTrack, prevTrack, shuffle, setShuffle, hstIdx, history, skipTime, setSkipTime, setPlayBackSpeed} =
    useStore();
  ...
  const copyLink = () => {
    ...
    url.searchParams.append('time', String(Math.floor(audioRef.current.currentTime)));
  ...
  return (
    <div className="flex flex-col align-top m-2 rounded-lg bg-primary-200 text-white">
          ...
          <div className="flex-1">
            <IconButton
              ...
              <div className="bg-btn rounded mx-1 h-6 w-12 flex-1">
                <img
                  src={shuffle ? '/playbackImgs/shuffle.svg' : '/playbackImgs/inorder.svg'}
          ...
          <div className="flex-1">
            <IconButton onClick={copyLink}>
              <div className="flex bg-btn rounded mx-1 h-6 w-12">
                <img src={'/playbackImgs/copy.svg'} className="w-full h-full" alt="copy" />
              </div>
            </IconButton>
          </div>
        </div>
      ...
      <div className="w-full flex justify-evenly  ">
        <PlayBackButtons imgSrc={'/playbackImgs/left.svg'} onClick={prevTrack} />
        <PlayBackButtons
          onClick={() => {
            if (audioRef.current) {
        ...
        <PlayBackButtons onClick={nextTrack} imgSrc={'/playbackImgs/right.svg'} />
      </div>
    </div>
  );
}
