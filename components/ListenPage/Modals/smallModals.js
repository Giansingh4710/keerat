...
import { useStore, useModalStore } from "utils/store.js";
...
export function ListOfArtistsModal() {
...
export function ListOfTypesModal() {
  const setCheckedType = useStore((state) => state.setCheckedType);
  const setCheckedArtist = useStore((state) => state.setCheckedArtist);
  const allOpts = useStore((state) => state.allOptsTracks);

  const artistToShowTypesFor = useModalStore(
    (state) => state.artistToShowTypesFor,
  );
  const setArtistToShowTypesFor = useModalStore(
    (state) => state.setArtistToShowTypesFor,
  );
            ...
            {tracksLst.map((obj, idx) => {
              ...
              return (
                <ArtistOptBar
                  ...
                  title={typeName}
                  toggleCheckbox={() =>
                    setCheckedType(artistToShowTypesFor, idx, !checked)
                  }
                  rightText={ratio}
                  onRightTextClick={() => {
...
export function ListOfTracksByType() {
  const allOpts = useStore((state) => state.allOptsTracks);
...
export function ViewTracksInQueueModal() {
