...
function titleFromLink(link: string): string {
  ...
  const lastPart = lst[lst.length - 1] ? lst[lst.length - 1] : lst[lst.length - 2];
  const firstChar = lastPart[0].toUpperCase();
  const idxWhereNewWord: number[] = [];
...
function LinkTag({href, absLink, children}: LinkTagProps): JSX.Element {
...
function NavMenu({children}: NavMenuProps): JSX.Element {
  const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null);
  ...
  return (
    <>
      <IconButton edge="start" color="inherit" aria-label="menu" onClick={handleClick} className="text-primary-100">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-10">
          <div className="flex items-center gap-6">
            ...
            <span className="text-primary-100 font-bold text-xl">{title}</span>
          </div>
