...
interface ButtonProps {
  onClick: () => void;
  children: ReactNode;
  color?: 'red' | 'blue' | 'green' | 'regular' | 'none';
  size?: 'sm' | 'md' | 'lg';
...
export function PlayBackButtons({onClick, imgSrc}: PlayBackButtonsProps): JSX.Element {
  return (
    <IconButton onClick={onClick}>
      <div className="h-10 w-10 flex-1">
        <img src={imgSrc} className="w-full h-full" alt="playback control" />
      </div>
    </IconButton>
  );
}
...
export const Button = ({
  ...
  const colorClasses: Record<string, string> = {
    ...
    green: 'bg-green-300 hover:bg-green-400 active:bg-green-500 border-green-500',
